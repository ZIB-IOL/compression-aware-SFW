...
def modular_sparsity(parameters_to_prune: List, refVal: Union[int, None] = None) -> float:
    ...
    for module, param_type in parameters_to_prune:
        if hasattr(module, param_type) and not isinstance(getattr(module, param_type), type(None)):
            ...
            n_zero += n_zero_param
    if refVal:
        return 1 - float(n_total) / refVal if refVal > 0 else 0
    else:
        return float(n_zero) / n_total if n_total > 0 else 0


def modular_compression(parameters_to_prune: List, refVal: Union[int, None] = None) -> float:
    """Returns the global modular compression rate of module"""
    s = modular_sparsity(parameters_to_prune=parameters_to_prune, refVal=refVal)
    if s == 1:
...
def global_sparsity(module: torch.nn.Module, param_type: Union[str, None] = None,
                    refVal: Union[int, None] = None) -> float:
    ...
    for name, module in module.named_modules():
        for param_type in param_list:
            if hasattr(module, param_type) and not isinstance(getattr(module, param_type), type(None)):
                ...
                n_zero += n_zero_param
    if refVal:
        # We have a reference value of total parameters of the non-decomposed model
        return 1 - float(n_total) / refVal
    else:
...
def compression_rate(module: torch.nn.Module, param_type: Union[str, None] = None,
                     refVal: Union[int, None] = None) -> dict:
    ...
    """
    s = global_sparsity(module, param_type=param_type, refVal=refVal)
    if s == 1:
        # The global sparsity is 1, hence the compression rate is infinitely large.
        return {}
    return 1. / (1 - s)
